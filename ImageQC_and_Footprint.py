...
class ImageFootprint:
    ...
    def GeoJSONFootprint(self):
        gdf = gpd.read_file(self.shp_footprint_path)
        GeoJSON_dict = mapping(gdf.geometry.iloc[0])
        GeoJSON_dict['coordinates'] = [list(map(list, ring)) for ring in GeoJSON_dict['coordinates']]
...
class QualityCheckFootprintTool:
    ...
    def __init__(self):
        ...
        self.UpdateSmartOLS = UpdateSmartOLS(self.PrePost, self.sensor_user_input, self.filename_word, self.availability, self.reception, self.DateTime,
                                             self.sensor_resolution, self.imageQCFolder)
    ...
    def RunTool(self):
        ...
        if processed == False:
            ...
            if abort_procedure == False:
                ...
                GeoJSONFootprint = self.ImageFootprint.GeoJSONFootprint()
                self.ImageFootprint.GDBFootprint()
                self.QualityCheckCompiler.Compiler()
                self.UpdateSmartOLS.polFootprint = GeoJSONFootprint
                self.UpdateSmartOLS.core()
            ...
            arcpy.AddMessage("All necessary files are already present in your system! Skipping directly to OLS submission...")
            abort_procedure = self.ImageFootprint.UploadOnToc()
            if abort_procedure == False:
                GeoJSONFootprint = self.ImageFootprint.GeoJSONFootprint()
                self.UpdateSmartOLS.polFootprint = GeoJSONFootprint
